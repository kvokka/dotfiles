...
- cass-memory rules live at two levels: the global store, and a repository's committed `.cass/playbook.yaml` (`fw:init` runs `cm init --repo` and adds `.cass/.gitignore`). With `projectRuleRouting: repo`, reflect writes a project rule into the `.cass/` of the session's main checkout when it exists, else into the global store scoped to that path. `cm context --workspace <main checkout>` merges both levels. cm itself runs from `$HOME` (the reflect job, the context hook), so no repository's `.cass/config.yaml` or logs are touched.
