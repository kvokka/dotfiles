...
- A managed config of a third-party tool carries only the values that differ from the tool's defaults, never a copy of its default or generated config. Check each key against the tool's source or docs, and leave out any key that equals its default. A default value is kept only when the tool would otherwise rewrite the file itself; a comment next to it names the reason. The one exception is `homedir/dot_config/ntm/config.toml`: the owner reads ntm's whole reference config there, so every default stays as a commented line and only our keys are live.
...
- Services (`agent-mail` on 0.0.0.0:8765 inside a private tmux server whose TUI `mise run fw:am-tui` opens, `cm` on 8766, `sbh`, and the cron jobs `cass-index` every 5 minutes, `cass-nightly` daily at 10:05, `cm-reflect` daily at 12:00, `am-subagent-reap` daily at 11:30, `ntm-web` on 127.0.0.1:7338 with `ntm-web-publish` forwarding the published port 7337 to it) are pitchfork daemons declared in `[daemons]`; the container has no systemd. Never run `am service install`, `sbh install`, `dcg install`, `agy install` or any upstream `install.sh`: chezmoi owns the configs, and the clients' MCP servers and user-scope hooks come from rulesync (next section).
...
- ntm works in `~/proj/active` (`projects_base`), and `ntm spawn <repo>` registers every pane in Agent Mail without setting a variable in it; the hooks, the guard and the agents take the name from `am agents resolve-pane`. The F6 palette and F12 dashboard keys are in the managed `tmux.conf`: never run `ntm bind`, which writes `~/.tmux.conf`. Notes: `homedir/dot_config/fw/templates/docs/agent-tooling/ntm.md` and `agent-mail.md`.
...
- Agent Mail hooks (`~/.config/fw/bin/am-hook`) respect what Claude Code and Codex show the model: plain stdout only from `SessionStart` (the agent's name, acks and reservations), and from `PostToolUse` only a JSON `additionalContext` (the `am check-inbox` reminder, rate-limited by am). Plain `PostToolUse` stdout, as in the hooks `am setup` writes, never reaches the model. The agent is `AGENT_NAME`, else `AGENT_MAIL_AGENT`, else the Agent Mail identity of the current tmux pane (`am agents resolve-pane`, the name ntm registered); inside a subagent (`agent_id` in the hook input) only the subagent's own identity; without one `am-hook` prints nothing.
- A harness subagent that may edit files is an Agent Mail participant of its own: `~/.config/fw/bin/am-subagent` (`SubagentStart`/`SubagentStop` hooks, Claude Code and Codex) creates its identity, hands it the name as `additionalContext` and releases its reservations when it stops; read-only agent types are skipped. The `am-subagent-reap` cron daemon retires identities unused for a day. `am agents reap` is not used: it would retire busy ntm pane agents. Note: `homedir/dot_config/fw/templates/docs/agent-tooling/agent-mail.md`.
