...
- Every MCP server is a shared pitchfork daemon reached over HTTP, one process per container, no secret in any client config. `morph` (WarpGrep, `@morphllm/morphmcp`) speaks only stdio, so `mcp-proxy` serves it on 8767 (one child for all sessions; `supergateway` starts one per request). Sharing one process is safe for `codebase_search`: it searches the `repo_path` of each call and ignores the process cwd and MCP roots, which only the disabled file tools use. `MORPH_API_KEY` comes from the environment (fnox), and `DISABLED_TOOLS` leaves only `codebase_search`.
