...
- **Frontmatter** (YAML): Contains `name` and `description` fields. These are the primary fields used for discovery. OpenCode also recognizes `license`, `compatibility`, and `metadata`.
...
- `name`: The skill name. Must be 1–64 characters, lowercase alphanumeric with single hyphen separators (`^[a-z0-9]+(-[a-z0-9]+)*$`). It must match the directory name.
- `description`: This is the primary triggering mechanism for your skill, and helps AI Agent understand when to use the skill.
  ...
  - Max 1024 characters.
- `compatibility`: (Optional) specify `opencode`.
- `license`: (Optional) specify the license.
- `metadata`: (Optional) string-to-string map for additional info.
