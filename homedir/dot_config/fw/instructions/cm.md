...
- Before non-trivial work, read the `<cm-context>` block if the session has one; otherwise call `cm_context(task="<one line>", workspace="<absolute repository root>")`, or run `cm context "<task>" --json` in the repository.
