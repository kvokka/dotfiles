...
| `ubs` | Ultimate Bug Scanner: multi-language pattern scanner tuned for generated code | Quality gate before every commit (pre-commit hook, findings fail the commit) | `ubs <changed files>` (exit 0 = clean); setup notes: `docs/agent-tooling/ubs.md` |
