...
- **The web dashboard works without login only through the forwarder.** ntm serves in auth mode `local` only on a loopback address; any other address needs an API key, and then the dashboard page and its websocket answer 401 in a browser. So ntm binds 127.0.0.1:7338 and socat publishes 7337. The compose file publishes 7337 on the host's loopback only: whoever reaches it can type into the agents' panes. The dashboard calls the API at `http://localhost:7337`, and ntm accepts only localhost origins, so open it as `localhost`, not through the OrbStack domain.
