...
(( $+commands[claude] )) && cc() { NODE_OPTIONS="--max-old-space-size=${CLAUDE_HEAP_MB:-16384}" command claude --dangerously-skip-permissions "$@"; }
(( $+commands[codex] ))  && cod() { command codex --dangerously-bypass-approvals-and-sandbox --search "$@"; }
